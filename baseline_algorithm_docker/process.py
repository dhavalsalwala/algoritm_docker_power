...
class MyAlgorithm(ShippingBaseAlgorithm):
    ...
    def load_model(self):
        ...
        for baseline in baselines:
            partition = baseline.split("_")[2]  # replace 2 with 6 when running in mac.

            # load scaling parameters
            self.mean[partition] = pd.Series(
                load_json(Path(baseline) / Path("stats") / Path("means.json"))
            )
            self.std[partition] = pd.Series(
                load_json(Path(baseline) / Path("stats") / Path("stds.json"))
            )

            members = glob.glob(str(Path(baseline) / Path("member_*")))

            self.list_of_models[partition] = []
            for member_path in members:
                model = ProbMCdropoutDNN(
                ...
                model_member_path = (
                    Path(baseline) / Path(member_path) / Path("best_model.pth")
                )
                model.load_state_dict(torch.load(model_member_path))
                model.to(device)
                self.list_of_models[partition].append(model)

    @staticmethod
    def form_output(time_id, power, uncertainties):
        return pd.DataFrame(
            {"time_id": time_id, "power": power, "uncertainty": uncertainties}
        )
    ...
    def predict(self, inputs: pd.DataFrame) -> pd.DataFrame:
        ...
        df_part1 = inputs[(inputs["awind_vcomp_provider"] <= 0)]
        df_part2 = inputs[
            (inputs["awind_vcomp_provider"] > 0)
            & (inputs["awind_vcomp_provider"] <= 10)
        ]
        df_part3 = inputs[
            (inputs["awind_vcomp_provider"] > 10)
            & (inputs["awind_vcomp_provider"] <= 20)
        ]
        df_part4 = inputs[
            (inputs["awind_vcomp_provider"] > 20)
            & (inputs["awind_vcomp_provider"] <= 30)
        ]
        df_part5 = inputs[(inputs["awind_vcomp_provider"] > 30)]
        ...
        for partition in ["P1", "P2", "P3", "P4", "P5"]:
            ...
            norm_inputs = data_normalization(
                data=inputs[self.input_features],
            ...
            tensor_inputs = (
                torch.tensor(norm_inputs[self.input_features].values).float().to(device)
            )

            preds_norm = get_ensemble_predictions(
                model_list=list_of_models, data_norm=tensor_inputs, multi_runs=10
            )

            # step 3. - denormalize predictions
            preds_denorm_local = denorm_prediction(
            ...
            if preds_denorm is None:
                ...
                preds_denorm = np.concatenate(
                    (preds_denorm, preds_denorm_local), axis=1
                )
        ...
        outputs = self.form_output(
            time_id=indexes, power=avg_predictions, uncertainties=uncertainties
        )
        return outputs
